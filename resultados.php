      ...
      <?php foreach ($por_jornada as $jornada => $ps): ?>
        ...
        <div class="partidos-list mb-3">
          ...
          <div class="partido-card">
            ...
            <!-- Equipo local -->
            <div class="partido-equipo <?= $p['ganador_id']==$p['equipo_local_id']?'text-navy fw-bold':'' ?>">
              <span class="partido-nombre"><?= epl_h($p['local_nombre']) ?></span>
              <?php if ($p['ganador_id']==$p['equipo_local_id']): ?>
                <span class="badge badge-jugado" style="font-size:.65rem">✓ Ganador</span>
              <?php endif; ?>
            </div>
            ...
            <div class="partido-resultado">
              ...
              <?php elseif ($p['estado'] === 'walkover'): ?>
                <span class="badge badge-walkover">Walkover</span>
              <?php elseif ($p['estado'] === 'reprogramado'): ?>
                <span class="badge badge-reprog">Reprogramado</span>
                <?php if ($p['fecha_programada']): ?>
                <span style="font-size:.72rem;color:var(--gray-400)"><?= date('d/m/Y H:i', strtotime($p['fecha_programada'])) ?></span>
                <?php endif; ?>
              <?php else: ?>
                <?php if ($p['fecha_programada']): ?>
                <span style="font-size:.8rem;font-weight:700;color:var(--gold)"><?= date('d/m/Y H:i', strtotime($p['fecha_programada'])) ?></span>
                <?php endif; ?>
            ...
            <!-- Equipo visitante -->
            <div class="partido-equipo right <?= $p['ganador_id']==$p['equipo_visitante_id']?'text-navy fw-bold':'' ?>">
